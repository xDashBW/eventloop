...
#define type_to_epoll(name, voidname) struct epoll *name = (struct epoll *)voidname
...
const struct pollops epollops = {epoll_new, epoll_delete, epoll_add, epoll_mod, epoll_del, epoll_dispatch};
...
static void *epoll_new(struct eventloop *loop) {
    ...
    epoll->epevs = (struct epoll_event *)malloc(sizeof(struct epoll_event) * EPOLL_EVENTS_INIT);
...
static void epoll_dispatch(void *ops) {
    ...
    if (n == epoll->epevs_sz) {
        epoll->epevs = realloc(epoll->epevs, sizeof(struct epoll_event) * 2 * n);
        assert(epoll->epevs);
